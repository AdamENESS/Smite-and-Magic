...
;CameraViewport camera,0,0,120,80
Dim pcol(120,80)
...
CameraFogColor camera,20,20,20
...
Next
For map.map=Each map
If map\envion=3 And map\area<50
CameraFogColor camera,200,200,255
CameraRange camera,1,65
CameraFogRange camera,5,55
...
End Select
If map\show=1
PositionEntity map\flor,EntityX#(map\flor)+x,EntityY#(map\flor),EntityZ#(map\flor)+z
ShowEntity map\mesh
PositionEntity map\mesh,x,0,z
PositionEntity map\coll,x,0,z
;FlipMesh map\coll
ShowEntity map\flor
ShowEntity map\coll
EntityType map\coll,TYPE_WALL
EntityAlpha map\coll,.4
EndIf
EndIf
...
HideEntity ground
HideEntity roof
PositionEntity camera,138,0,-64
...
;a=a+1
;	;	For a=0 To 255
;			If EntityInView (map\mesh,camera)
;				Text a,a*16,map\name$+"In view Dist:"+EntityDistance(camera,map\mesh)
;				If EntityDistance(camera,map\mesh)<64 
;				ShowEntity map\mesh
;				EndIf
;				
;			Else
;				HideEntity map\mesh
;				Text a,a*16,map\name$+"Not In view Dist:"+EntityDistance(camera,map\mesh)
;			EndIf
;		Next
;	EndIf
;Next
...
;	If x>15
;	ClearCollisions
	;PositionEntity camera,-1,EntityY#(camera),EntityZ#(camera)
;	Exit
	;EndIf

	If KeyDown( 205 ) TurnEntity camera,0,-1,0
	If KeyDown( 203 ) TurnEntity camera,0,1,0
	If KeyDown( 208 ) MoveEntity camera,0,0,-0.05*(width/2) ;Collisions TYPE_PLAYER,TYPE_WALL,2,3
	If KeyDown( 200 ) MoveEntity camera,0,0, 0.05*(width/2) ;Collisions TYPE_PLAYER,TYPE_WALL,2,3
	If KeyDown(2) 
		For map.map=Each map
			If map\envion=3	EntityAlpha map\coll,0
		Next
		End If
	If KeyDown(3)
			For map.map=Each map
			If map\envion=3 EntityAlpha map\coll,.4
		Next
		EndIf
...
;	EndIf
	...
	;Text 0,0,"Area:"+map\name+" X:"+Int(EntityX(camera)/width)+" Z:"+Int(EntityZ(camera)/width+.5)
;	For map.map=Each map
;	DebugLog EntityDistance(camera,map\mesh)
;	If EntityDistance(camera,map\mesh)>32
;	HideEntity map\mesh
;	HideEntity map\roof
;	Else
;	ShowEntity map\mesh
...
z=Int(EntityZ(camera)/width+.5)
area$=""
	Text 0,16," Collision:"+collission+" FPS:"+fps()
draw(mode)
Flip
If KeyHit(12) mode=mode-1
If KeyHit(13) mode=mode+1
...
If z>=0 And z<=15
	...
	xm=16
	zm=0
	Else If x>=32 And x<=47
	area="C1"
	xm=32
	zm=0
	Else If x>=48 And x<=63
	area="D1"
	xm=48
	zm=0
	Else If x>=64 And x<=80
	area="E1"
	xm=64
	zm=0
	End If
Else If z>=-15 And z<=0
 If x>=15 And x<=31
	area="A2"
	xm=0
	zm=-16
...
Else If z>=48 And z<=63
	...
	End If
EndIf
Text 0,0,"Area:"+area+" X:"+(x-xm)+" Z:"+(z-zm)


Function loadmap()
mapd=OpenFile("map.data")
...
LockBuffer BackBuffer()
	...
	Color 0,0,0
	Rect 0,0,120,90
	If mode<8 LockBuffer BackBuffer()
	For c=0 To 120
		For d=0 To 90
...
;smile
If mode=1
WritePixelFast a*8+1,b*8,pcol(c,d)
WritePixelFast a*8+4,b*8,pcol(c,d)
WritePixelFast a*8+1,b*8+3,pcol(c,d)
WritePixelFast a*8+4,b*8+3,pcol(c,d)
WritePixelFast a*8+2,b*8+4,pcol(c,d)
WritePixelFast a*8+3,b*8+4,pcol(c,d)
Else If mode>1 And mode<4
WritePixelFast a*8,b*8,pcol(c,d)

WritePixelFast a*8+1,b*8+1,pcol(c,d)
WritePixelFast a*8+1,b*8-1,pcol(c,d)
WritePixelFast a*8-1,b*8+1,pcol(c,d)
WritePixelFast a*8-1,b*8-1,pcol(c,d)
	If mode=3
		WritePixelFast a*8+2,b*8+2,pcol(c,d)
		WritePixelFast a*8+2,b*8-2,pcol(c,d)
		WritePixelFast a*8-2,b*8+2,pcol(c,d)
		WritePixelFast a*8-2,b*8-2,pcol(c,d)

		WritePixelFast a*8+1,b*8,pcol(c,d)
		WritePixelFast a*8,b*8-1,pcol(c,d)
		WritePixelFast a*8,b*8+1,pcol(c,d)
		WritePixelFast a*8-1,b*8,pcol(c,d)
	EndIf
	Else If mode=4
	...
	WritePixelFast a*8+x,b*8+z,pcol(c,d)
	Next
	Next
	Else If	mode=5
...
Next
Next
If mode<8 UnlockBuffer (BackBuffer)
EndIf
End Function
