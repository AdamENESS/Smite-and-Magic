;=========================================================================
;XLNT GUI INTERFACE V1.84
;A SIMPLE TO USE GUI FOR BLITZ BASIC PROGRAMS
;WRITTEN BY YAPPY (STARTED 11.04.01) (EMAIL:stew.yapp@btinternet.com)
...
;SUPPLIED IN THE XLNT-GUI DISTRIBUTION PACK. HOWEVER, THESE FILES ARE FREELY DISTRIBUTABLE 
;VIA NON-COMMERCIAL MEANS AS LONG AS ALL FILES REMAIN INTACT. 
...
;==>Include "XLNT-GUI_v1.84.BB"		;INCLUDE THE GUI CODE

;Graphics 800,600,0,1		;SETUP OUR GRAPHICS DISPLAY

;PLACE THE FOLLOWING LINE AFTER YOU HAVE SET UP YOUR GRAPHICS DISPLAY
;AND BEFORE YOU CALL ANY GUI COMMANDS
;==>GUI_GFXSETUP()
...
;=========================================================================
;GUI WORLD VARIABLES
Global GUI_SCREENW=800			;DEFAULT SCREEN WIDTH 
Global GUI_SCREENH=600			;DEFAULT SCREEN HITE
...
Function GUI_SELECTOR.GADGET(T.WINDOW,X,Y,W,A$,ACT,HELP$)
	...
	If MAXHITE>5
		If GAD\SELECTOR\MAXL<20
			GAD\SELECTOR\LINES=GAD\SELECTOR\MAXL
		Else
			GAD\SELECTOR\LINES=20
		EndIf
	Else
...
Function GUI_SKIN_GAD(GAD.GADGET,INV=0)
	...
	X=GAD\X:Y=GAD\Y:C$=GAD\CAPTION$:ON=GAD\ON:ACT=GAD\ACTIVE
	;SetBuffer ImageBuffer(WIN\IMAGE)
	Select GAD\GADTYPE
		Case 0		;GUI_BUT
		;	SKN=WIN\SKIN1
		;	GUI_DRAWBUTTUN(X,Y,W,H,C$,0,ACT,SKN)
		Case 1		;ON/OFF
		Case 2		;GUI_CYCLE
		Case 3		;GUI_TXTINPUT
			If GAD\TXTINPUT\LABEL$<>""
				X=X+GAD\TXTINPUT\OX;StringWidth(GAD\TXTINPUT\LABEL$)+2
			EndIf
		;	GUI_DARKBUT(X,Y,W,H,WIN\SKIN0_NORM)
		;	GUI_TEXTINPUTPRINT(WIN,GAD)
		Case 4		;GUI_TXTBOX
		Case 5		;GUI_SLIDER
		Case 6		;GUI_INTEGER
...
Function GUI_HILITE(X,Y,W,H,PLUS=16)
	BUFF=GraphicsBuffer()
	LockBuffer BUFF
	For YY=Y To Y+H
		For XX=X To X+W
			COL=ReadPixelFast(XX,YY,BUFF) And $FFFFFF
			RD=(COL Shr 16) And 255
			GR=(COL Shr 8) And 255
			BL=COL And 255
			RD=QLIMIT(RD+PLUS,0,255):GR=QLIMIT(GR+PLUS,0,255):BL=QLIMIT(BL+PLUS,0,255)
			COL=(RD Shl 16) + (GR Shl 8) + BL
			WritePixelFast XX,YY,COL,BUFF
		Next
	Next
	UnlockBuffer BUFF
End Function

Function GUI_LOLITE(X,Y,W,H,PLUS=16)
	BUFF=GraphicsBuffer()
	LockBuffer BUFF
	For YY=Y To Y+H
		For XX=X To X+W
...
Function GUI_SHOWHLP(X,Y,HELP$)
	...
	For XX=X+1 To X+W-2
		For YY=Y+1 To Y+H-2
			COL=ReadPixel(XX,YY,BackBuffer())
			R=(COL Shr 16) And 255
			GAD=(COL Shr 8) And 255
			B=COL And 255
			R=QLIMIT(R-70,30,255)
			GAD=QLIMIT(GAD-60,30,255)
			B=QLIMIT(B-30,70,255)
			COL=(R Shl 16)+(GAD Shl 8)+B  
			WritePixel XX,YY,COL,BackBuffer()
		Next
	Next
	;UnlockBuffer BackBuffer()
	...
	If WIN<>Null
		...
		Select GUI_WINEVENT$
			...
			Case "DRAG"
				...
				YOF=MouseY()-WIN\Y
				Cls
				SetBuffer BackBuffer():Origin 0,0
				If GUI_BACK_ON<>0 And GUI_BG<>0
					Viewport GUI_BG_VX,GUI_BG_VY,GUI_BG_VW,GUI_BG_VH
					DrawBlock GUI_BG,GUI_BG_X,GUI_BG_Y
					Viewport 0,0,GUI_SCREENW,GUI_SCREENH
